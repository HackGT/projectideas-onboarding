...
ideaRoutes.route("/remove/:id").post(async (req, res, next) => {
  await Idea.findByIdAndRemove({ _id: mongoose.Types.ObjectId(req.body.id)}).exec();
...
ideaRoutes.route("/edit/:id").post(async (req, res, next) => {
  const resp = await Idea.updateOne({ _id: req.params.id}, { title: req.body.title, description: req.body.description})
